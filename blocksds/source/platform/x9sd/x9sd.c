...
static void X9SD_SDSendCommand(u8 cmd, u32 parameter, void* buffer, u32 num_words) {
    cardExt_RomReadData(X9SD_CMD_SDIO(cmd, parameter), X9SD_CTRL_READ_512B, buffer, num_words);
}

// TODO: what is this?
static void X9SD_CardSendCommand58(u8 parameter) {
    cardExt_RomSendCommand(X9SD_CMD_58(parameter), X9SD_CTRL_POLL);
}

// TODO: what is this?
static void X9SD_CardSendCommand5C(u32 parameter) {
    cardExt_RomSendCommand(X9SD_CMD_5C(parameter), X9SD_CTRL_POLL);
}

// TODO: what is this?
static void X9SD_CardSendCommand5D(u32 parameter) {
    cardExt_RomSendCommand(X9SD_CMD_5D(parameter), X9SD_CTRL_POLL);
}

// TODO: what is this?
static void X9SD_CardSendCommand63(void) {
    cardExt_RomReadData(X9SD_CMD_63, X9SD_CTRL_READ_512B_DELAY, NULL, 0);
}
...
void X9SD_SDReadSector(u32 sector, u8* buffer) {
    ...
    cardExt_RomReadData(X9SD_CMD_SDIO(17, sector), X9SD_CTRL_READ_512B, scratch, 128);
    ...
    while (!data_ready) {
        cardExt_RomReadData(X9SD_CMD_CARD_DATA_READ(sector), X9SD_CTRL_READ_512B, scratch, 128);
        for (i = 0; i < 512; i++) {
            if (!(scratch[i] & 0xF0)) {
                // We found our data. Let's move on
                data_ready = true;
    ...
    cardExt_RomReadData(X9SD_CMD_CARD_DATA_READ(sector), X9SD_CTRL_READ_1KB, scratch + 0x200, 256);
    ...
    cardExt_RomReadData(X9SD_CMD_CARD_DATA_READ(sector), X9SD_CTRL_READ_512B, NULL, 0);
}
...
void X9SD_SDWriteSector(u32 sector, const void* buffer) {
    ...
    cardExt_RomWriteData(X9SD_CMD_CARD_DATA_WRITE(0), X9SD_CTRL_WRITE_512B, buffer, 128);
